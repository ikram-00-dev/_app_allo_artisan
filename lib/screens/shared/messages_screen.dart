...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  Future<void> fetchMessages(int contactId) async {
    try {
      ...
      final response = await ApiService.get('/messages/$contactId');

      final List<Message> loadedMessages = (response as List)
  ...
  Future<void> sendMessage() async {
    if (messageController.text.trim().isEmpty || selectedContact == null) {
      return;
    }
    ...
    try {
      final data = {
        ...
        "contactId": selectedContact["ID"] ?? selectedContact["id"],
        "senderId": currentUserId,
        "seen": false,
      };

      final response = await ApiService.post('/messages', data);
  ...
  List<dynamic> get filteredContacts {
    return contacts.where((contact) {
      final name = (contact["Username"] ?? contact["username"] ?? "")
          .toString()
          .toLowerCase();
